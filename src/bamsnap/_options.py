...
def postprocessing_option(opt):
    ...
    if 'out' in opt.keys() and ((not 'logfile' in opt.keys()) or (opt['logfile'] == "")):
...
def check_option(opt):
    ...
    if ('out' in ks and opt['out'] == ''):
        ...
        elif opt['bamlist'] != None:
            opt['out'] += opt['bamlist'].split('/')[-1]
        opt['out'] += '_' + opt['pos'].replace(':','_').replace('-','_')

    if (not 'bam' in ks or opt['bam']==None) and (not 'bamlist' in ks or opt['bamlist']==None) :
        opt['log'].error('Please insert bam file or bam file list with -bam or -bamlist option.')
        has_opt_error = True

    if (not 'pos' in ks or opt['pos']==None) and (not 'vcf' in ks or opt['vcf']==None) and (not 'bed' in ks or opt['bed']==None):
...
def get_options():
    parser = argparse.ArgumentParser(usage='%(prog)s <sub-command> [options]', description='%(prog)s ver' + conf.VERSION + " (" + conf.VERSION_DATE + ")" + ': convert bam to image')
    parser.add_argument('-v', '--version', action='version', version='%(prog)s ver' + conf.VERSION + " (" + conf.VERSION_DATE + ")")
    ...
    parser.add_argument('-pos', dest='pos', default=[], help='genomic position (ex. 1:816687-818057, 12:7462545)', nargs="*")
    parser.add_argument('-vcf', dest='vcf', default=None, help='list file with genomic positions with VCF format')
    parser.add_argument('-bed', dest='bed', default=None, help='list file with genomic positions with BED format')
    parser.add_argument('-out', dest='out', default='', help='title of output file')
    parser.add_argument('-out_type', dest='out_type', choices=['png','jpg','image'], default='png', help='output file type')
    ...
    parser.add_argument('-draw', dest='draw', default="coverage,heatmap,read", help='plot (default: -draw coverage,heatmap,read )')
    ...
    parser.add_argument('-coverage_vaf', dest='coverage_vaf', default=10, type=float, help='coverage variant allele fraction threshold (unit:%%)')
    ...
    if 'conf' in opt.keys() and opt['conf'] != None and util.is_exist(opt['conf']):
