...
class BamSnap():
    ...
    def load_bamlist(self):
        ks = self.opt.keys()
        if 'bamlist' in ks and self.opt['bamlist']!=None:
    ...
    def save_image(self, ia, bam, pos1):
        outfname = os.path.join(self.opt['out'],'snapfiles', 'snap_' + bam.split('/')[-1] + '_' + pos1['chrom'] + '_' + str(pos1['t_spos']) + '-' + str(pos1['t_epos']))
        check_dir(outfname)
        ...
        self.outfnamelist.append({'t':bam.split('/')[-1] + ': ' + pos1['chrom'] + '_' + str(pos1['t_spos']) + '-' + str(pos1['t_epos']),'img':outfname.split('/')[-1]})
        ...
        if not self.has_opt_error:
            ...
            for pos1 in self.opt['poslist']:
                ...
                for bam in self.bamlist:
                    ...
                    for plot1 in self.opt['draw'].split(','):
                        if plot1 == "heatmap":
                            rset.draw_coverage_heatmap(drawA,[(0,offset_top),(image_w,offset_top+self.opt['heatmap_height'])])
                            self.scale_x = rset.scale_x
                            offset_top += self.opt['heatmap_height']

                        if plot1 == "coverage":
                            rset.draw_coverage(drawA,[(0,offset_top),(image_w,offset_top+self.opt['coverage_height'])])
                            self.scale_x = rset.scale_x
                ...
                if self.opt['merged_image']:
                    self.save_image(ia, bam, pos1)
                t13 = time.time()
                self.opt['log'].debug('Running time for processing position '+pos1['chrom']+':'+str(pos1['t_spos'])+'-'+str(pos1['t_epos']) + ": " + str(round(t13-t11,5))+' sec')
        t2 = time.time()
        self.save_html()
        

        self.opt['log'].debug('Total running time for getting reference sequence (set_refseq): ' + str(round(timemap['set_refseq'],3))+' sec')
